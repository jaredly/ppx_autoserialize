...
let suffix = "__'bs'to_json";

let variant_converter param_names constructors => {
  simple (Longident.Lident "jkjk")
};

let rec suffixify lident => Longident.(switch lident {
| Lident x => Lident (x ^ suffix)
| Ldot x y => Ldot x (y ^ suffix)
| Lapply a b => Lapply a (suffixify b)
});
...
let rec record_converter param_names labels => {
  open Parsetree;
  open Longident;
  open Ast_helper;
...
and core_type_converter typ => {
  open Parsetree;
  open Ast_helper;
  switch typ.ptyp_desc {
  | Ptyp_constr {txt} args => {
    let main = simple (suffixify txt);
    if (args === []) {
      main
    } else {
      Exp.apply main (List.map (fun arg => (Asttypes.Nolabel, core_type_converter arg)) args)
    }
  }
  | Ptyp_var name => {
...
let make_converters {Parsetree.ptype_name: {txt}, ptype_params, ptype_kind, ptype_manifest, } => {
  ...
  | Some typ => {
    (core_type_converter typ)
  }
  | None => switch ptype_kind {
    | Ptype_abstract => [%expr fun value => "type is abstract"]
    | Ptype_variant constructors => variant_converter param_names constructors /* TODO names */
    | Ptype_record labels => record_converter param_names labels
  ...
  structure: fun mapper structure => {
    let rec loop items => {
      ...
      | [{pstr_desc: Pstr_type isrec declarations} as item, ...rest] => {
        let converters = (List.map make_converters declarations |> List.concat);
