...
let config = {
  prefix: [%str
    ...
    let list__to_devtools = (convert, items) =>
      {"$bs": "list", "items": List.map(convert, items) |> Array.of_list} |> to_devtools;
  ...
  record: (core_type_converter, labels, name) => {
    ...
    let sets =
      List.map(
        ({pld_name: {txt}, pld_type}) => {
          ...
          [%expr
            Js.Dict.set(result, [%e strConst(txt)], [%e core_type_converter(pld_type)]([%e value]))
          ];
        },
        labels
      );
    let body =
      List.append(
        ...
        [
          [%expr
            {"$bs": "record", "type": [%e strConst(name)], "attributes": result |> to_devtools}
            |> to_devtools
          ]
        ]
      )
